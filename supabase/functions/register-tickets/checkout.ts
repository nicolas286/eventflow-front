import type { CheckoutContext, RegisterRequestBody, RuntimeConfig } from "./types.ts";
import { badRequest } from "./errors.ts";
import { appendQueryParams, getSafeWidgetReturnUrl } from "./origins.ts";

export function resolveCheckoutContextOrThrow(
  ...
  const widgetReturnUrl =
    checkoutSource === "widget"
      ? getSafeWidgetReturnUrl(body.widgetReturnUrl, config.allowedOrigins)
      : null;
