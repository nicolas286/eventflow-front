...
const MAX_WIDGET_RETURN_URL_LENGTH = 2048;
...
function parseBoundedString(v, maxLength, errorCode = "INVALID_PAYLOAD") {
...
function parseCheckoutSource(v) {
  const s = toOptionalTrimmedString(v);
...
function parseBuyer(input) {
  ...
  const isAttendee = typeof obj.isAttendee === "boolean" ? obj.isAttendee : undefined;
...
export async function parseRegisterPayload(req) {
  ...
  const turnstileToken = parseBoundedString(raw.turnstileToken, MAX_TURNSTILE_TOKEN_LENGTH, "MISSING_CAPTCHA_TOKEN");
  if (!turnstileToken) {
  ...
  const widgetReturnUrl = parseBoundedString(raw.widgetReturnUrl, MAX_WIDGET_RETURN_URL_LENGTH, "INVALID_PAYLOAD");
  return {
    eventId: raw.eventId,
    items,
    attendees,
