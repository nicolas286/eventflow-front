...
export async function findReusablePayment(admin, orderId) {
  const { data, error } = await admin.from("payments").select("provider_payment_id, raw, created_at").eq("order_id", orderId).eq("provider", "mollie").in("status", [
    "open",
    "pending"
