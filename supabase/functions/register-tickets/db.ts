...
export async function getEventPaymentContextOrThrow(admin, eventId) {
  const { data, error } = await admin.from("events").select("org_id, title").eq("id", eventId).maybeSingle();
...
export async function getOrgPlanOrThrow(admin, orgId) {
  const { data, error } = await admin.from("organizations").select("plan").eq("id", orgId).maybeSingle();
  if (error) {
