...
function toNonEmpty(v: string | null) {
  const s = (v ?? "").trim();
  return s ? s : null;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!isValidUuid(orderId)) return json({ error: "INVALID_ORDER_ID" }, 400);
    if (!bookingToken) return json({ error: "MISSING_TOKEN" }, 401);
    ...
    if (oErr) return json({ error: "DB_ERROR", details: oErr.message }, 500);
    if (!oRow) return json({ error: "NOT_FOUND" }, 404);
