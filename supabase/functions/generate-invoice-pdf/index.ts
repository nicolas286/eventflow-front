...
function moneyFromCents(cents: number, currency = "EUR") {
...
async function buildPdfBytes(invoice: any) {
  ...
  const vat = [billing?.vatCountryCode, billing?.vatNumber].filter(Boolean).join(" ") || "-";
  ...
  // Big title
  page.drawText("FACTURE", { x: margin, y: yLeft, size: 30, font: fontBold });
  yLeft -= 40;

  // Seller block (left)
  page.drawText("Emetteur", { x: margin, y: yLeft, size: 12, font: fontBold });
  yLeft -= 16;

  for (const line of sellerLines) {
    page.drawText(line, { x: margin, y: yLeft, size: 10, font });
    yLeft -= 13;
  }

  // Invoice meta (left)
  yLeft -= 10;
  page.drawText(`N° E-${number}`, { x: margin, y: yLeft, size: 12, font: fontBold });
  yLeft -= 18;

  page.drawText(`Emise le : ${issuedAt || "-"}`, { x: margin, y: yLeft, size: 10, font });
  yLeft -= 14;

  page.drawText(`Payee le : ${paidAt || "-"}`, { x: margin, y: yLeft, size: 10, font });
  yLeft -= 14;

  page.drawText(`Periode : ${periodStart || "-"} - ${periodEnd || "-"}`, { x: margin, y: yLeft, size: 10, font });
  yLeft -= 18;
  ...
  page.drawText("Client", { x: bx, y: yRight, size: 12, font: fontBold });
  yRight -= 16;
  ...
  for (const line of buyerLines) {
    page.drawText(line, { x: bx, y: yRight, size: 10, font });
    yRight -= 13;
  }
  ...
  page.drawText("Description", { x: col1, y, size: 10, font: fontBold });
  page.drawText("Montant", { x: col2, y, size: 10, font: fontBold });
  y -= 16;

  page.drawText("Abonnement EventFlow", { x: col1, y, size: 10, font });
  page.drawText(moneyFromCents(total, currency), { x: col2, y, size: 10, font });
  y -= 28;

  // Totals (right)
  const tx = width - margin - 200;
  page.drawText(`Sous-total : ${moneyFromCents(subtotal, currency)}`, { x: tx, y, size: 10, font });
  y -= 14;
  page.drawText(`TVA : ${moneyFromCents(vatCents, currency)}`, { x: tx, y, size: 10, font });
  y -= 16;
  page.drawText(`Total : ${moneyFromCents(total, currency)}`, { x: tx, y, size: 12, font: fontBold });

  // Footer: VAT exemption + generated
  page.drawText(vatExemptionLine, { x: margin, y: margin + 10, size: 9, font });
  page.drawText("Document genere automatiquement.", { x: margin, y: margin - 5, size: 9, font });
...
Deno.serve(async (req) => {
  try {
    ...
    if (!invoiceId || !isUuid(invoiceId)) {
      return json({ error: "VALIDATION_ERROR: invoice_id invalid" }, 400);
    }
    ...
    const { data: inv, error: invErr } = await admin
      .from("invoices")
      .select("id, org_id, number, currency, subtotal_cents, vat_cents, total_cents, issued_at, paid_at, period_start, period_end, billing_snapshot, pdf_path")
      .eq("id", invoiceId)
      .maybeSingle();

    if (invErr) return json({ error: "DB_ERROR", details: String(invErr.message ?? invErr) }, 500);
    if (!inv?.id) return json({ error: "NOT_FOUND" }, 404);

    if (!inv.org_id || !inv.number) {
      return json({ error: "VALIDATION_ERROR: invoice missing org_id/number" }, 400);
    }
...
const accountingPath = `accounting/${safeYear}/${inv.org_id}-${inv.number}.pdf`;
    ...
    try {
      ...
      return json({ error: "PDF_BUILD_FAILED", details: String((e as any)?.message ?? e) }, 500);
    }

// Upload options
const uploadOpts = {
  contentType: "application/pdf",
  upsert: force, // si force=true -> overwrite, sinon immutable
};
...
if (upClient.error && !force) {
  ...
  if (!isAlreadyExists) {
    return json({ error: "UPLOAD_FAILED_CLIENT", details: msg }, 500);
  }
}
...
if (upAccounting.error && !force) {
  ...
  if (!isAlreadyExists) {
    return json({ error: "UPLOAD_FAILED_PERSO", details: msg }, 500);
  }
}
...
if (!inv.pdf_path || force) {
  ...
  if (setErr) {
    return json({ error: "SET_PDF_PATH_FAILED", details: String(setErr.message ?? setErr) }, 500);
  }
}

    return json({ ok: true, invoice_id: inv.id, pdf_path: clientPath }, 200);
  } catch (e) {
