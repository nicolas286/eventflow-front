...
Deno.serve(async (req)=>{
  try {
    ...
    if (!supabaseUrl || !anonKey || !serviceKey) return json({
    ...
    if (!invoiceId || !isUuid(invoiceId)) return json({
      error: "invoice_id invalid"
    }, 400);
    ...
    const { data: inv, error: invErr } = await admin.from("invoices").select("id, org_id, pdf_path").eq("id", invoiceId).maybeSingle();
    ...
    if (!isMember) return json({
      error: "FORBIDDEN"
    }, 403);
    // 3) signed url (1-5 minutes, comme tu veux)
    const expiresIn = 120; // 2 min
    const { data: signed, error: signErr } = await admin.storage.from("invoices").createSignedUrl(inv.pdf_path, expiresIn);
    if (signErr || !signed?.signedUrl) {
      return json({
        error: "SIGN_FAILED",
        details: signErr?.message ?? "no_signed_url"
      }, 500);
    }
    return json({
