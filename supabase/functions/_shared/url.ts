export function normalizeOrigin(input: string) {
  const url = new URL(input);
  return `${url.protocol}//${url.host}`;
}

export function parseAllowedOrigins(raw: unknown) {
  return String(raw ?? "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean)
    .map((s) => {
      try {
        return normalizeOrigin(s);
      } catch {
        return "";
      }
    })
    .filter(Boolean);
}

export function resolveAppBaseUrlFromRequest(
  req: Request,
  allowedOrigins: string[],
) {
  const origin = req.headers.get("origin");

  if (origin) {
    try {
      const normalized = normalizeOrigin(origin);
      if (allowedOrigins.includes(normalized)) return normalized;
    } catch {
      // ignore invalid origin
    }
  }

  const referer = req.headers.get("referer");

  if (referer) {
    try {
      const normalized = normalizeOrigin(referer);
      if (allowedOrigins.includes(normalized)) return normalized;
    } catch {
      // ignore invalid referer
    }
  }

  return null;
}

export function getSafeReturnUrl(opts: {
  raw: unknown;
  allowedOrigins: string[];
  allowedPathPrefix?: string;
}) {
  const value = typeof opts.raw === "string" ? opts.raw.trim() : "";

  if (!value) return null;

  try {
    const url = new URL(value);
    const origin = `${url.protocol}//${url.host}`;

    if (!opts.allowedOrigins.includes(origin)) return null;

    if (
      opts.allowedPathPrefix &&
      !url.pathname.startsWith(opts.allowedPathPrefix)
    ) {
      return null;
    }

    return url.toString();
  } catch {
    return null;
  }
}

export function appendQueryParams(
  baseUrl: string,
  params: Record<string, unknown>,
) {
  const url = new URL(baseUrl);

  for (const [key, value] of Object.entries(params)) {
    if (value != null && String(value).trim()) {
      url.searchParams.set(key, String(value));
    }
  }

  return url.toString();
}