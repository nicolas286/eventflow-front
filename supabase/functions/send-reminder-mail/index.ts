...
import { sendMail as sendSharedMail } from "../_shared/mail/mailService.ts";
...
/**
 * Retourne YYYY-MM-DD en Europe/Brussels.
 * Sert à comparer les jours civils côté rappel.
 */
...
async function sendMail(payload) {
  const result = await sendSharedMail({
    to: payload.to,
    subject: payload.subject,
    html: payload.isHtml ? payload.content : undefined,
    text: payload.isHtml ? undefined : payload.content,
    tags: {
      kind: "reminder_v1",
  ...
  if (!result.ok) {
    console.error("[send-reminder-mail] shared mail service failed", {
...
function shouldSendToday(startsAtIso, reminderDays) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const r of orgProfiles ?? []) {
      reminderByOrgId.set(
        String(r.org_id),
        Number(r.email_reminder_days_before ?? 0),
      );
    }
    ...
    for (const o of orders ?? []) {
      ...
      const discountCents = await loadOrderDiscountCents(admin, orderId);
      const dueCents = Math.max(0, totalCents - discountCents - paidCents);
...
async function loadOrderDiscountCents(admin, orderId) {
  ...
  if (error) {
    console.error("[send-reminder-mail] load promo_code_redemptions failed", error);
    return 0;
  }

  return (data ?? []).reduce((sum, r) => {
    const n = Number(r?.discount_cents ?? 0);
...
async function processOneOrder(admin, appBaseUrl, orderId, debug, prefetched) {
  ...
  let discountCents = prefetched?.discountCents ?? 0;
  let paidCents = prefetched?.paidCents ?? 0;
  let dueCents = prefetched?.dueCents ?? 0;
  ...
  if (!prefetched) {
    ...
    discountCents = await loadOrderDiscountCents(admin, orderId);
  ...
  const orderUrl = `${appBaseUrl}/order/${orderId}?token=${
    encodeURIComponent(bookingToken)
  }`;
