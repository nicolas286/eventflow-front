 ...
 */
function brusselsDateKey(d: Date) {
...
/* ---------------- template HTML reminder (aligné sur confirmation) ---------------- */
...
function buildOrderReminderHtml(p: ReminderHtmlParams) {
...
Deno.serve(async (req) => {
  ...
  let body: SendReminderInput = { mode: "cron" };
  try {
  ...
  try {
    ...
    if ((body as any)?.mode === "manual") {
      ...
      return json({ ok: true, mode: "manual", ...res });
    }
    ...
    const { data: orders, error } = await admin
      ...
      `)
      .in("status", ["created", "pending", "paid", "confirmed"]) // adapte si besoin
      .not("event_id", "is", null)
      .limit(limit);

    if (error) {
      console.error("[send-reminder-mail] load orders failed", error);
      return json({ ok: false, error: "LOAD_FAILED" }, 502);
    }
    ...
    const orgIds = Array.from(
      new Set(
        (orders ?? [])
          .map((o: any) => o?.events?.org_id)
          .filter(Boolean)
          .map((x: any) => String(x)),
      ),
    );

    const { data: orgProfiles, error: opErr } = await admin
    ...
    if (opErr) {
      ...
      return json({ ok: false, error: "LOAD_ORG_PROFILE_FAILED" }, 502);
    }

    const reminderByOrgId = new Map<string, number>();
    for (const r of orgProfiles ?? []) {
      reminderByOrgId.set(
        String((r as any).org_id),
        Number((r as any).email_reminder_days_before ?? 0),
      );
    }
    ...
    for (const o of orders ?? []) {
      ...
      const buyerEmail = String((o as any)?.buyer_email ?? "").trim();
      const bookingToken = String((o as any)?.booking_token ?? "").trim();
      ...
      const currency = String((o as any)?.currency ?? "EUR").trim() || "EUR";
      const totalCents = Number((o as any)?.total_cents ?? 0) || 0;
      const paidCents = Number((o as any)?.paid_cents ?? 0) || 0;
...
async function loadOrderItems(admin: ReturnType<typeof createClient>, orderId: string): Promise<OrderItemSnap[]> {
...
async function processOneOrder(
...
): Promise<{ status: "sent" | "already_sent" | "invalid"; detail?: any }> {
  // 1) idempotence
  ...
  if (logErr) {
    ...
    return { status: "invalid", detail: "log_failed" };
  }
  if (!logged) return { status: "already_sent" };
  ...
  let startsAt: string | null = prefetched?.startsAt ?? null;
  let location: string | null = prefetched?.location ?? null;
  let description: string | null = prefetched?.description ?? null;
  ...
  if (!prefetched) {
    ...
    if (oErr || !o?.id) return { status: "invalid", detail: "order_not_found" };

    buyerEmail = String((o as any).buyer_email ?? "").trim();
    bookingToken = String((o as any).booking_token ?? "").trim();
    currency = String((o as any).currency ?? "EUR").trim() || "EUR";
    totalCents = Number((o as any).total_cents ?? 0) || 0;
    paidCents = Number((o as any).paid_cents ?? 0) || 0;

    if ((o as any).event_id) {
      const { data: ev } = await admin
        .from("events")
        .select("title, description, starts_at, location")
        .eq("id", String((o as any).event_id))
        .maybeSingle();
  ...
  if (!looksLikeEmail(buyerEmail) || !bookingToken || !startsAt) {
    return { status: "invalid", detail: "missing_fields" };
  }
  ...
  if (debug) {
    return {
      status: "sent",
      detail: { to: buyerEmail, subject, orderUrl, startsAt, reminderDays, currency, totalCents, paidCents, itemsCount: items.length },
    };
  }

  await sendMail({ to: buyerEmail, subject, content: html, isHtml: true });
  return { status: "sent" };
}