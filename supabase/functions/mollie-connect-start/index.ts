...
type Payload = { orgId: string; mode: "test" | "live" };
...
function extractState(st: unknown): string | null {
...
Deno.serve(async (req) => {
  ...
  try {
    if (req.method !== "POST") {
      return json(req, { error: "VALIDATION_ERROR: method_not_allowed" }, 405);
    }
    ...
    const scopes = (Deno.env.get("MOLLIE_CONNECT_SCOPES") ?? "")
      .trim()
      .replace(/\s+/g, " ");

    if (!supabaseUrl || !anonKey || !clientId || !redirectUri || !scopes) {
      return json(req, { error: "UNKNOWN: missing_env" }, 500);
    }
    ...
    if (uErr || !u?.user) return json(req, { error: "NOT_AUTHENTICATED" }, 401);

    const returnBaseUrl = resolveReturnBaseUrl(req);
        if (!returnBaseUrl) return json(req, { error: "ORIGIN_NOT_ALLOWED" }, 403);
    ...
    if (stErr) {
      return json(req, { error: stErr.message }, 400);
    }

    const state = extractState(st);
    if (!state) return json(req, { error: "UNKNOWN: state_missing" }, 500);
    ...
    return json(req, { error: "UNKNOWN: unexpected" }, 500);
  }
});
