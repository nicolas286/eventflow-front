...
/* ------------------------------------------------------------------ */ /* Edge handler                                                       */ /* ------------------------------------------------------------------ */ Deno.serve(async (req)=>{
  try {
    /* -------------------- */ /* Preflight / method  */ /* -------------------- */ if (req.method === "OPTIONS") {
    ...
    /* -------------------- */ /* Cron auth            */ /* -------------------- */ const expected = envTrim("CRON_SECRET");
    ...
    /* -------------------- */ /* Supabase admin       */ /* -------------------- */ const supabaseUrl = envTrim("SUPABASE_URL");
    ...
    /* -------------------- */ /* Business logic       */ /* -------------------- */ const { data, error } = await admin.rpc("expire_orders", {
      p_limit: 200
    });
    if (error) {
      return json({
